        ...
        for (const product of productsBaskets) {
            ...
            if (visible)
                if (product.myProduct.quantity > 0) {
                    ...
                    const response = await fetch(`${apiBaseUrl}/favorite/artworks/${checkCookie("user12345")}/${product.myProduct.id}`);
                    if(!response.ok) {
                        template += ` <div class="liked-icon-form">
                    ...
                    <div class="not-liked-icon-form">
                        <button type="submit" onclick="this.handleClick(event)"  class="not-liked-icon heart-circle-not-liked-heart"><i
                            id="not-liked-heart1"
