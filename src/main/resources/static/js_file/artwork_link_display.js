    ...
    try {
        ...
        function navigateArtwork(element1, element2) {
            getElement(element1).onclick = () => {
                element1 === "#next"
                    ? (window.location.href = "/artworks/00000" + (Number(cookieNum2) + 1))
                    : (window.location.href = "/artworks/00000" + (Number(cookieNum2) - 1));
...
makeFetchSynchronous().then((data) => {
    ...
    proceed_to_checkout.addEventListener("click", (event) => {
        ...
        if (formValidation.allinputValid) {
            const loader = document.querySelector(".loading-checkout") || document.querySelector(".loader");
            if (loader) {
            ...
            fetch(`${apiBaseUrl}/cart/checkout/create-checkout-session`, {
                ...
                .then((resp) => {
                    console.log(resp);
                    if (!resp.ok) {
                        throw new Error("Sorry something is wrong,please try again later" + resp);
                    }
                    return resp.text();
                })
                .then((sessionId) => {
...
function showContent(headline, reviewText, reviewerName, date, rate, imageData) {
    ...
    for (let i = 0; i <= rate; i++) {
        if (star_reviewed[i]) star_reviewed[i].src = "/images/icons/star_review.svg";
    }
...
function getRate() {
    ...
    for (let index = 0; index < allStar.length; index++) {
        allStar[index].onclick = () => {
            ...
            getElement("#rate-num").value = index;
        };
    }
}

// Reset star UI after successful submit
function resetStarsUI() {
    const blanks = document.querySelectorAll(".star-blank");
    blanks.forEach((s) => {
        s.src = "/images/icons/star_blank.svg";
    });
    const rateInput = document.querySelector("#rate-num");
    if (rateInput) rateInput.value = "";
}

// File upload handler (kept)
function handleFileUpload(event) {
    const file = event.target.files[0];
    if (file && file.type.startsWith("image/")) {
        window.uploadedFile = { file, dataURL: null };
...
function myData() {
    getElement("#form-review").onsubmit = async (event) => {
        event.preventDefault();
        ...
        const rate = Number(getElement("#rate-num").value);
        ...
        const whileWaiting = getElement(".while-waiting");
        if (whileWaiting) whileWaiting.style.display = "none";

        try {
            const resp = await fetch(`${apiBaseUrl}/product/review/save`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-IDENTIFIER": userId.substring(userId.length / 2),
                    "X-CSRF-TOKEN": document.querySelector(".csrf-token").value,
                },
                body: JSON.stringify({
                    headline,
            ...
            if (!resp.ok) {
                ...
                // prefer server echo; fall back to optimistic values
                let saved = null;
                try {
                    saved = await resp.json();
                } catch (_) {}

                const headlineOut = saved?.headline ?? headline;
                const textOut = saved?.reviewText ?? comment;
                const nameOut = saved?.reviewer?.fullName ?? "You";
                const dateOut = saved?.date ?? new Date().toISOString();
                const rateOut = Number(saved?.rating ?? rate);
                const imageOut = saved?.imageData ?? imageData;

                // Append the new review instantly (no reload)
                showContent(headlineOut, textOut, nameOut, dateOut, rateOut, imageOut);
                ...
                // Reset form + stars + preview
                const formEl = document.querySelector("#form-review");
                if (formEl) formEl.reset();
                resetStarsUI();
                window.uploadedFile = null;

                console.log("comment added successfully (no reload)");
            }
        } catch (e) {
            console.error(e);
            alert("Unable to submit your review right now.");
            if (loader) {
                loader.classList.add("loader--hidden");
                loader.addEventListener(
                    "transitionend",
...
(function submitForm() {
    ...
    getRate();
    document.querySelector(".file-image").addEventListener("change", handleFileUpload);
    myData();
})();
